...
public class DossierController extends BaseController {
    ...
    private List<CarDossier> prepareData(List<CarDossier> list) {
        for (CarDossier carDossier : list) {
            ...
            map.put("linkId", carDossier.getId());
            map.put("recordStatus", ContextType.PAY_RECORD_BUDGET);
            carDossier.setBudgetList(daoCenter.getCarPaidRecordList(map));

            map.clear();
            map.put("linkId", carDossier.getId());
            map.put("remarkType", ContextType.REMARK_TYPE_DOSSIER);
            carDossier.setRemarkList(daoCenter.getCarRemarkList(map));
        }
        return list;
    }
    ...
    public ModelAndView dossierView(HttpSession session) {
        //同步数据
        CommonAction.syncCarRecord(daoCenter);

        ModelAndView mav = new ModelAndView();
        mav.setViewName("dossier/list");
        Map<String, Integer> map = new HashMap<String, Integer>();
        map.put("displayStatus", ContextType.DISPLAY_STATUS_SHOW);
        mav.addObject(DATA, prepareData(daoCenter.getCarDossierByMap(map)));
    ...
    public ModelAndView dossierDoneView(HttpSession session) {
        ...
        mav.setViewName("dossier/list");
        Map<String, Integer> map = new HashMap<String, Integer>();
        map.put("displayStatus", ContextType.DISPLAY_STATUS_DONE);
        mav.addObject(DATA, prepareData(daoCenter.getCarDossierByMap(map)));
        mav.addObject(TIP, session.getAttribute("tip"));
    ...
    public ModelAndView dossierAction(
        ...
        if (over == null) {
            if (id != null) {
                CarDossier carDossier = daoCenter.getCarDossier(id);
                CarRecord carRecord = daoCenter.getCarRecordById(carDossier.getCarRecordId());
                mav.addObject("id", id);
            ...
            if (action.equals(CREATE_ACTION)) {
                ...
                carDossier.setId(id);
                daoCenter.updateCarDossier(carDossier);
            }
