...
public class UserController extends BaseController {
    ...
    public ModelAndView userList(HttpSession session) {
        ...
        try {
            List<Account> dataList = daoCenter.getAccountList(null);

            List<Account> respList = new ArrayList<Account>();
            for (Account tempAccount : dataList) {
                tempAccount.setDepartmentName(daoCenter.getDepartmentAuthorityById(tempAccount.getDepartment()).getDepartmentName());
//				//自己帐号直接添加
//				if(account.getName().equals(tempAccount.getName())) respList.add(tempAccount);
//				//其他帐号权限判断
//				if(account.getDepartment()< tempAccount.getDepartment()){
...
//				}
    ...
    public ModelAndView userAdd(
        ...
        if (daoCenter.getAccountByName(accountNum) != null) {
        ...
        if (purchaseCommission == null) purchaseCommission = 0;
        ...
        account.setActive(1);
        daoCenter.createAccount(account);
        mav.clear();
    ...
    public ModelAndView userUpdateView(
        ...
        Account account = daoCenter.getAccountById(id);
        if (account == null) {
    ...
    public ModelAndView userUpdate(
        ...
        if (!StringUtils.isEmpty(name)) account.setName(name);
        if (!StringUtils.isEmpty(phone)) account.setPhone(phone);
        account.setUtime(System.currentTimeMillis());
        daoCenter.updateAccountById(account);


        Account sAccount = (Account) session.getAttribute("account");
        boolean isMyself = sAccount.getId().equals(id);
        if (isMyself) {
            session.setAttribute("account", daoCenter.getAccountById(id));
        }
    ...
    public ModelAndView deleteUser(
        ...
        daoCenter.deleteAccountById(id);
